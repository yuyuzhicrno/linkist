...
channelsRouter.get('/:id', (req, res) => {
  const ch = db.channels.find(c => c.id === req.params.id || c.slug === req.params.id);
  if (!ch) return res.status(404).json({ error: 'Channel not found' });
  const messages = ch.messages.map(m => ({
    ...m,
    author: (() => { const u = db.users.find(x => x.id === m.authorId); return u ? { id: u.id, username: u.username, avatar: u.avatar } : null; })()
  }));
...
channelsRouter.post('/:id/messages', (req, res) => {
  ...
  res.json({ ...msg, author: { id: user.id, username: user.username, avatar: user.avatar } });
});
