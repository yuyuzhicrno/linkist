...
import { db, calcLevel, CHANNEL_CREATE_LEVEL, addXp, flushDatabase, recordDbOp } from '../data/db.js';
...
channelsRouter.post('/', validate(schemas.createChannel), (req, res) => {
  const user = getUser(req);
  ...
  const channel = {
    ...
    slug: sanitizeInput(name).toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, ''),
...
channelsRouter.post('/:id/join', (req, res) => {
  ...
  recordDbOp('update', 'channels', ch.id);
  flushDatabase();
  res.json({ success: true });
});

// POST send message
channelsRouter.post('/:id/messages', validate(schemas.sendMessage), (req, res) => {
  const user = getUser(req);
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
  const ch = db.data.channels.find(c => c.id === req.params.id);
  if (!ch) return res.status(404).json({ error: 'Channel not found' });
  if (!ch.isPublic && !ch.memberIds.includes(user.id) && ch.ownerId !== user.id && user.role !== 'admin')
    return res.status(403).json({ error: '只有频道成员才能发送消息' });
  const { content } = req.body;
  const msg = { id: uuidv4(), authorId: user.id, content: sanitizeInput(content).trim(), createdAt: new Date().toISOString(), reactions: {} };
...
channelsRouter.post('/:channelId/messages/:msgId/react', (req, res) => {
  ...
  if (!ch) return res.status(404).json({ error: 'Channel not found' });
  if (!ch.isPublic && !ch.memberIds.includes(user.id) && ch.ownerId !== user.id && user.role !== 'admin')
    return res.status(403).json({ error: '只有频道成员才能反应消息' });
