...
import { db, safeUser, addXp } from '../data/db.js';
...
friendsRouter.get('/', (req, res) => {
  ...
  const friends = (me.friends || []).map(fid => safeUser(db.users.find(u => u.id === fid))).filter(Boolean);
  const requests = (me.friendRequests || []).map(rid => safeUser(db.users.find(u => u.id === rid))).filter(Boolean);
...
friendsRouter.post('/request/:targetId', (req, res) => {
  const me = getUser(req);
  if (!me) return res.status(401).json({ error: 'Unauthorized' });
  const target = db.users.find(u => u.id === req.params.targetId);
...
friendsRouter.post('/accept/:requesterId', (req, res) => {
  const me = getUser(req);
  if (!me) return res.status(401).json({ error: 'Unauthorized' });
  const requester = db.users.find(u => u.id === req.params.requesterId);
...
friendsRouter.delete('/remove/:otherId', (req, res) => {
  const me = getUser(req);
  if (!me) return res.status(401).json({ error: 'Unauthorized' });
  const other = db.users.find(u => u.id === req.params.otherId);
...
friendsRouter.post('/dms/:userId', (req, res) => {
  ...
  addXp(me.id, 1);
  res.json({ ...msg, sender: { id: me.id, username: me.username, avatar: me.avatar } });
});
