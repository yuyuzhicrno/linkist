...
dbRouter.get('/stats', (req, res) => {
  ...
  const totalMessages = db.channels.reduce((s, c) => s + c.messages.length, 0);
  
  const allTags = db.posts.flatMap(p => p.tags);
  const tagCounts = {};
...
dbRouter.get('/categories', (req, res) => {
  const cats = [...new Set(db.posts.map(p => p.category))];
  res.json(cats.map(c => ({
    name: c,
    count: db.posts.filter(p => p.category === c).length
  })));
});
